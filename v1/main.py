...
authors_n = []
authors_a = []
date_ = [1801,1712,1711,1710,1709,1708,1707,1706,1705,1704,1703,1702,1701]
max_n = 15000
len_max = 5
for d in date_:
	for i in range(1,max_n):
	    ...
	    if "doesn't exist" in html_doc:
        	    break 
	    subject = ex.extract_subject(html_doc)
	...
	se5 = pd.Series(subjects)    
	df = pd.DataFrame(columns=['title','abstracts','authors_n','authors_a','subject'])
	df['title'] = se.values
	df['abstracts'] = se2.values
	df['authors_n'] = se3.values
	df['authors_a'] = se4.values
	df['subject'] = se5.values
	df.to_csv('/Users/alivalehi/Dropbox/project/Dataincubator/v1'+str(d)+'.csv')
